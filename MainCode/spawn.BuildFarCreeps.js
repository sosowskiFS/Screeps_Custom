var spawn_BuildFarCreeps = {
    run: function(spawn, thisRoom, energyIndex) {
        if (!spawn.spawning && thisRoom.storage && Memory.roomsUnderAttack.indexOf(thisRoom.name) == -1) {
            ...
            if (Memory.warMode) {
                ...
                if (Memory.guardType) {
                    if (thisRoom.energyCapacityAvailable >= 1900) {
                        farGuardConfig = [MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, HEAL];
                    } else if (thisRoom.energyCapacityAvailable >= 1280) {
                        farGuardConfig = [MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, RANGED_ATTACK, HEAL];
                    }
                } else {
            ...
            if (Game.flags[thisRoom.name + "FarMineral"] && thisRoom.terminal && prioritizedRole == '') {
                if (farMineralMiners.length < 1 && blockedRole != 'farMineralMiner') {
                    ...
                    farMinerConfig = [CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,WORK,WORK,WORK,WORK,WORK,WORK,WORK,WORK,WORK,WORK,WORK,WORK,WORK,WORK,WORK,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE];
                }
            }

            if (Game.flags[thisRoom.name + "FarMineral2"] && thisRoom.terminal && prioritizedRole == '') {
            ...
            if (Game.flags[thisRoom.name + "FarMineral3"] && thisRoom.terminal && prioritizedRole == '') {
