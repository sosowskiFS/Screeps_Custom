			...
			if (creep.room.name != creep.memory.destination) {
				...
				if (creep.room.controller.reservation && (creep.room.name == creep.memory.destination)) {
					if (creep.room.controller.reservation.ticksToEnd <= 1000) {
						if (Memory.FarClaimerNeeded[creep.memory.homeRoom].indexOf(creep.room.name) == -1) {
							Memory.FarClaimerNeeded[creep.memory.homeRoom] = Memory.FarGuardNeeded[creep.memory.homeRoom] + creep.room.name;
						}
					} else {
						if (Memory.FarClaimerNeeded[creep.memory.homeRoom].indexOf(creep.room.name) != -1) {
							Memory.FarClaimerNeeded[creep.memory.homeRoom] = Memory.FarGuardNeeded[creep.memory.homeRoom].replace(creep.room.name, "");
						}
					}
