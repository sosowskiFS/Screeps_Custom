		...
		switch (creep.memory.priority) {
			...
			case 'farGuardNearDeath':
				...
				if (creep.room.controller && creep.room.controller.owner && creep.room.controller.owner.username != "Montblanc" && creep.room.name != creep.memory.destination) {
					...
					if ((!Memory.warMode && closeFoe.owner.username != "ThyReaper" && closeFoe.getActiveBodyparts(ATTACK) == 0 && closeFoe.getActiveBodyparts(RANGED_ATTACK) == 0) || Memory.warMode) {
						...
						if (creep.pos.getRangeTo(closeFoe) > 3 || (closeFoe.getActiveBodyparts(ATTACK) > 1) || (creep.getActiveBodyparts(RANGED_ATTACK) == 0) || (creep.room.controller && creep.room.controller.safeMode)) {
